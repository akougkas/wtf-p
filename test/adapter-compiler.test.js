...
record('every command-capable host exposes the same stable aliases', () => {
  ...
  for (const target of ['clio', 'claude', 'copilot', 'opencode', 'antigravity', 'gemini']) {
    ...
    for (const [index, sourcePath] of expected.entries()) {
      ...
      const firstText = source.replace(/^---\n[\s\S]*?\n---\n/, '').replace(/^[^\n]*prompt = '''\n/m, '').trimStart();
      assert.ok(!firstText.startsWith('<!-- Generated by WTF-P'), `${target}:${sourcePath}: banner must not be the first body text`);
...
record('Codex, Claude, Copilot, Antigravity, Gemini, and plugin manifests resolve locally', () => {
  ...
  assert.strictEqual(codexManifest.name, 'wtfp');
  ...
  assert.strictEqual(antigravityManifest.name, 'wtfp');
  ...
  assert.deepStrictEqual(codexMarketplace.plugins[0].source, { source: 'local', path: './plugins/wtfp' });
  ...
  assert.strictEqual(copilotMarketplace.plugins[0].source, './plugins/wtfp');
