...
Read: `${PLUGIN_ROOT}/catalog.json`, `${PLUGIN_ROOT}/actions/{action}.json`.
Produce: none.
...
4. Read `${PLUGIN_ROOT}/compatibility/action-availability.json` and mark every action it reports as unavailable, naming the capability or effect that blocks it. Do not imply capabilities that the selected adapter does not expose.
