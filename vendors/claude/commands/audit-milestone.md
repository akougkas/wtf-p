...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/wtfp-tool.js <command> [arguments]
```

Run it with `list` first to read the declared commands, their arguments, and their bounds; `${CLAUDE_PLUGIN_ROOT}/tools/README.md` carries the same table. Never execute another module in this package, never pass a logical `project://` or `wtfp://` URI as a shell argument, and treat every returned record as candidate evidence until it is verified and written to a source or evidence record.
...
@${CLAUDE_PLUGIN_ROOT}/actions/audit-milestone.json
