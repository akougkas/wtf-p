...
2. Interview for identity, argument, document type, audience, requirements, exclusions, format, gates, safety, and author decisions. Once the document type is known, propose the initial section set from `${CLAUDE_PLUGIN_ROOT}/templates/paper-outline.md`, or from `${CLAUDE_PLUGIN_ROOT}/templates/grant-proposal-outline.md` for `grant-proposal`, which additionally requires a captured solicitation artifact before any section is proposed.
