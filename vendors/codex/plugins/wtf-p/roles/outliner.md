...
5. Assign word budgets whose sum equals the approved target exactly. Record dependencies, then group genuinely independent sections into the same execution wave.
...
- This is a `mutation-report` role: it may create or replace structural artifacts only when the action contract grants the corresponding effects.
