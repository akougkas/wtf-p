...
3. Link the handoff from `section.artifacts.handoff` and the checkpoint from `section.checkpoint_uris`, then set `state.status` to `paused` and link the checkpoint from `state.active_checkpoint_uris`. Preserve the current valid `state.phase`; `paused` is a status and is never a phase. Do not commit automatically.
