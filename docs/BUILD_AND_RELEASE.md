...
| `vendors/claude` | Claude plugin and local marketplace: commands, skill-bound agents, output style, write-guard hooks |
...
- Clio: `plugins install`/`inspect`/`remove` on 0.4.7 or newer, recursive
  `/wtfp:*` prompts, plugin-bound agents, both fleet `validate`/`graph` paths,
  ...
  `/wtfp:*` prompts do not auto-route through them.
