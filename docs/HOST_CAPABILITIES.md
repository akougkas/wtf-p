...
A third round ran on 2026-09-17 against the packed `0.7.0` candidate (`wtf-p-0.7.0.tgz`, packed from commit `0b105a3`, whose `vendors/`, `protocol/`, and `bin/` match `git archive` of that commit). Each envelope gained exactly one file, `tools/support/cite-nexus-client.js`. Every host ran under `env -i` with `HOME`, `XDG_*`, `TMPDIR`, and its client variable in a disposable directory and no forwarded credentials. The versions were `claude` 2.1.274, `codex` 0.153.3, a local `clio-coder` 0.4.9 build (source HEAD `0ba61280`, clean; not an npm release), `agy` 1.2.5, and temporary-prefix `copilot` 1.0.85, `opencode` 1.18.31, and `gemini` 0.60.0. On every host the installer route, native listing, and uninstall passed, and each listing showed `wtfp` at 0.7.0 where the host prints a version. The observed installer file counts were Claude 259, Codex 182, Clio 204, Copilot 420, OpenCode 203 (a same-harness `0.6.0` install gives 202; the 203 recorded below dates from rc.4), Antigravity 205, and Gemini 205. Claude strict validation, Clio `library validate`/`inspect` on `vendors/plugin` and `vendors/claude`, `agy plugin validate`, and `gemini extensions validate` passed. Claude, Codex, and Copilot also installed from a repository export through `plugin marketplace add`. OpenCode discovery ran from a disposable Git repository because, outside a Git root, OpenCode walks parent directories for skill folders. In-session discovery on Copilot and Gemini and runtime loading of Codex TOML agents were not checked. No model call was made in this round.

The same round exercised the CiteNexus backend by invoking the dispatcher from the installed Clio and Claude plugins by hand; no model drove these runs. With a disposable CiteNexus 0.2.0 companion installed from its prepared wheel before that package reached PyPI, one live Crossref query per host exited 0 with three candidates and empty `metadata.errors`, and no rate limit was reported. `--offline` exited 1 with the `network.search` refusal, and a nonexistent `WTFP_CITE_NEXUS_COMMAND` exited 1 with the install message and empty stdout. Action availability stayed 31/36 on Clio and Claude and 26/36 with `tool.execute` unbound on the other five hosts.
